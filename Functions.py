...
def generate_training_data(prm_top_files, traj_files, frames_i, frames_f, backbone, output_dir, scaling_factor=1.):
    ...
    for i_l, label in enumerate(list(range(len(prm_top_files)))):
        ...
        ref_u = u
        ref_u.trajectory[0]
        ref_pos = ref_u.select_atoms(
            backbone).positions - ref_u.atoms.center_of_mass()  # backbone works with both proteins and DNA
        ...
        for ts in u.trajectory[frames_i[i_l]:frames_f[i_l]:1]:
            ...
            if (ts.frame == 0):
                write_inpcrd(sel.positions / scaling_factor,
                             outname=output_dir + 'initial_lab' + str(label) + '.inpcrd')
...
def write_inpcrd(inp_tensor, outname='out.inpcrd'):
    ...
    for at in range(len(inp_tensor)):
        outf.write(" {:11.7f} {:11.7f} {:11.7f}".format(float(inp_tensor[at][0]), float(inp_tensor[at][1]),
                                                        float(inp_tensor[at][2])))
        if (at % 2 != 0):
            outf.write('\n')
    outf.close()


def generate_new_inpcrds(ddpm, prm_top_file, output_directory, n_samples=1, device=None, ats=71, dims=3, what_label=0,
                         scaling_factor=1.):
        ...
        for idx, t in enumerate(list(range(ddpm.n_steps))[::-1]):
            ...
            if idx in frame_idxs or t == 0:
                ...
                for i, x0 in enumerate(x):
                    outname = output_directory + 'output_frame-' + str(idx) + '_sample-' + str(i) + '_lab' + str(
                        what_label[0].item()) + '.inpcrd'
