...
parser.add_argument('--input_directory', default=r'C:\Users\bismark\Downloads\Protine_Data\\', type=str)
...
parser.add_argument('--N_classes', default=2,
                    type=int)  # Number of classes
parser.add_argument('--desired_labels', default=[0, 1], type=list)  # Classes to be considered for output

# Output parameters
parser.add_argument('--output_directory', default=r'C:\Users\bismark\Downloads\Protine_Data', type=str)
...
if __name__ == "__main__":
    ...
    if args.biosystem == 'DNA':
        backbone = 'name P'
    print(f"{frames_f=}")
    dataset = generate_training_data(prm_top_files=prmfs, traj_files=trajfs, frames_i=[0, 0], frames_f=frames_f,
                                     backbone=backbone, output_dir=args.output_directory,
    ...
    if args.input_shape == 'batch_3Nat':
        ddpm = inpcrd_DDPM_cond(
            inpcrd_Seq_Conditioned(max_size=box_s, n_steps=args.n_steps, time_emb_dim=args.time_emb_dim, Natoms=N_at,
                                   dims=3, label_emb_dim=args.label_emb_dim), n_steps=args.n_steps,
            min_beta=args.min_beta, max_beta=args.max_beta, device=device)

    if not args.no_train:
        if args.load_for_train:
            ddpm.load_state_dict(torch.load(store_path, map_location=device))
        training_loop(ddpm, loader, args.n_epochs, optim=Adam(ddpm.parameters(), args.learning_rate), device=device,
                      output_directory=args.output_directory, store_path=store_path)
    ...
    for i_l, label in enumerate(list(range(len(prmfs)))):
        generate_new_inpcrds(ddpm, prmfs[i_l], output_directory=args.output_directory, n_samples=args.num_samples,
                             device=device, ats=N_at, dims=3,
                             what_label=torch.tensor(args.num_samples * [i_l]).to(device),
                             scaling_factor=args.scaling_factor)
